    ...
    const getMakulYangDiampu = async () => {
        try {
            if (username && kodeTahun && kodeSemester && kodeJenjang && kodeFakultas && kodeProdi) {
                const response = await axios.get(`v1/jurnalDosen/getMakulByDosen/${username}/${kodeTahun}/${kodeSemester}/${kodeJenjang}/${kodeFakultas}/${kodeProdi}`)
