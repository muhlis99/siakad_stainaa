...
const ListRekapPersemester = () => {
    ...
    const getRekapPersemester = async () => {
        try {
            if (kodeTahun && kodeSemester && kodeJenjang && kodeFakultas && kodeProdi) {
                const response = await axios.get(`v1/presensiDosen/rekapPresensiPersmt/${kodeTahun}/${kodeSemester}/${kodeJenjang}/${kodeFakultas}/${kodeProdi}`)
                                ...
                                {dataRekap.map((item, index) => (
                                    <tr key={index} className='bg-white border-b text-gray-500 border-x'>
                                        ...
                                        <td className='px-3 py-2 font-semibold border' align='center'>
                                            <Link
                                                to={`/presensi/detailrekappersemester/${item.nip_ynaa}/${kodeTahun}/${kodeSemester}/${kodeJenjang}/${kodeFakultas}/${kodeProdi}`}
                                                state={{
                                                    collaps: 'kuliah',
                                                    activ: '/presensi',
                                                    select: 'persemester',
...
export default ListRekapPersemester