...
module.exports = {
    ...
    put: async (req, res, next) => {
        ...
        console.log(daysRoom);
        

        if (!validasiJamMulai) return res.status(401).json({ message: "JAM MULAI Jadwal kuliah bentrok" })
        if (!validasiJamSelesai) return res.status(401).json({ message: "JAM SELESAI Jadwal kuliah bentrok" })
        if (daysRoom) return res.status(401).json({ message: "HARI Jadwal kuliah bentrok" })
